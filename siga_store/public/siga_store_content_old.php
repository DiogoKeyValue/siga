    ...
    jQuery(document).ready(function () {
        ...
        */
                    ...
                    success: function (data) {
                        ...
                        $.each(json_obj['jsStoreDadosArtigosBeanRESTful']['jsArtigosBeanRestFuls'], function (key, value) {
                            ...
                            if(value.listaImagensWeb == null){
                                ...
                                $.each(listaImagens, function(key, imagem){
                                    ...
                                    if(key===0){
                                        ...
                                        html_img_thumb += '<img class="group list-group-image img-thumbnail thumb_pic_box_cat ct_img slideImg" descImgRef="' + imagem.designacaoImagem + '" src="' + imgThumb + '" data-key="' + key +'" alt="" data-product-id="' + value.CArtigo + '" />'
                                    } else{
                                        html_img_big += '<input type="hidden" name="path_pic" value="' + imgBig + '" class="ct_img_big" data-key="' + key + '">'
                                        html_img_thumb += '<img class="group list-group-image img-thumbnail thumb_pic_box_cat ct_img slideImg scrollHide" descImgRef="' + imagem.designacaoImagem + '" src="' + imgThumb + '" data-key="' + key +'" alt="" data-product-id="' + value.CArtigo + '" />'
                                    }
                            ...
                            if(value.desigComer!==undefined){
                                designacao_comercial='Cx=[' + value.desigComer + ']';
                            }
                            ...
                            '<div class="thumbnail product-item-' + value.CArtigo + '" '+addedProductBackground+'>'+
                                html_imagem +
                                '<div class="caption">'+
                                    '<h4 class="group inner list-group-item-heading" style="margin-bottom: 0;">'+
                                        '<span class="ct_id">Referência:' + value.CArtigo + '</span><br/>'+
                                        '<span class="cBarras">EAN: ' + value.CBarras + '</span>' + 
                                        (value.CExterno != null? ' / <span class="cExterno">' + value.CExterno + '</span>':'') +  
                                        '<span class="desig_comer" style="display:block; margin-top: 5px; margin-bottom: 10px; text-transform: initial;min-height:48px">'+designacao_comercial+'</span>'+

                                    '</h4>'+
                                    '<p class="group inner list-group-item-text">'+
                                        '<span class="ct_name">' + value.desigCArtigo + '</span>'+
                                    '</p>' + price_html + 
                                    '<input type="hidden" name="uni_med" class="ct_uni" value="' + value.cUniMed + '">'+
                                    '<div class="row">'+
                                        '<div class="col-md-4 col-xs-12 col-sm-4">' +
                                            '<input type="number" class="form-control math_cell cell_val_input upVal ct_qtd" value="'+ productQuantity +'" placeholder="Qtd" data-product-id="' + value.CArtigo + '" min="1" maxlength="11" id="qtd_' + value.CArtigo + '" ' + hide_price_status + ' >'+
        ...
        $(document).on('click', '.thumb_pic_box_cat', function (event) {
            ...
            if (objProduct !== null) {
                ...
                $.ajax({
                    ...
                    $('#myModal_cat .modal-dialog .modal-content .modal-footer').html('<div class="col-md-12 text-center">Cx=[' + data + ']</div>' + price_html);
                    ...
                    if (jqXHR.status == 200) {
                        ...
                        $('#myModal_cat .modal-dialog .modal-content .modal-footer').html('<div class="col-md-12">Cx=[' + jqXHR.responseText + ']</div>' + price_html);
                        $('#myModal_cat').modal('show');
                    }
                });
            }
        ...
        $(document).on('click', '.thumb_pic_box_enc', function (event) {
            ...
            $('#myModal_enc .modal-dialog .modal-content .modal-footer').html('<div class="container" style="'+modal_w+'">  '+$(this).parents('.item').find(".col-md-12").html()+'</div>');
        ...
        function addProductToCart(product, productId){
            ...
            if (searchCartItemById(productId, product.CLote, product.refimagens) === null) {
                ...
                $.each(cookie_cart, function(key, value){
                    ...
                    if(value.CArtigo === product.CArtigo){
                            ...
                            '<div class="thumbnail">'+
                                ...
                                '<div class="caption">'+
                                    '<h4 class="group inner list-group-item-heading" style="margin-bottom: 15px;">'+
                                        '<span class="ct_id">Referência: ' + value.codArtigo + '</span><br/>'+
                                        '<span class="cBarras">EAN: ' + value.codBarras + '</span>' + 
                                    '</h4>'+
                                    '<p class="group inner list-group-item-text">'+
                                    ...
                                    '<div class="row">'+
                                        '<div class="col-md-4 col-xs-12 col-sm-4">' +
                                            ...
                                            '" data-product-cloteint="' + (value.CLoteInt != null? value.CLoteInt : '') + '" value="' + value.qtd + '" placeholder="Qtd" min="1" maxlength="11" ' +
                                            'data-product-refimg="' + (value.refimagens != null ? value.refimagens : '') + '">'+
                                        '</div>'+
                                        '<div class="col-md-8 col-xs-12 col-sm-8">' +
            ...
            console.log(cartProducts)
                ...
                var jsonToPost = {
                    encomendasClienteBeanRESTful: {
                        ...
                        nrEncomenda: null,
                        suaEncomenda: "Portal-" + sigaStoreUsername + "-" + Date.now(),
