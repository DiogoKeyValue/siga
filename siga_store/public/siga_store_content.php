<script type="text/javascript">
    ...
    jQuery(document).ready(function () {
        ...
        */
        ...
        function check_btns_display(btn_current_status, tab_menu) {
            ...
            if (btn_current_status == 'list' || btn_current_status == '') {
                ...
                if(tab_menu == 'solidaria' || tab_menu == 'none') {
                    $("#products_cart_Change_to_List").addClass("active");
                    $('#products_cart .item').removeClass('grid-group-item');
                    $('#products_cart .item').addClass('list-group-item');
                }
        ...
        */
        ...
        function ajax_call_to_table(request_call) {
            ...
            if (homeSelect === null || homeSelect === "") {
                ...
                var artigosCatalogo = false;//!"#
                if(artigosCatalogo){
                    service_path = '/seam/resource/rest/jsStoreRestFulService/obterArtigosCatalogoPorCombos/';
                }else{
                ...
                var jsonValue = {
                    jsStoreQueryArtigoBeanRESTful: {
                        ...
                        obterSoDisponiveis: false, //stocks confirmaStockArm
                ...
                //Filtros dos tabs
                jsonValue.jsStoreQueryArtigoBeanRESTful.filtroColuna1NotNull = filtro1;
                jsonValue.jsStoreQueryArtigoBeanRESTful.valorfiltroColuna1 = valorFiltro1;

                jsonValue.jsStoreQueryArtigoBeanRESTful.filtroColuna2NotNull = filtro2;
                jsonValue.jsStoreQueryArtigoBeanRESTful.valorfiltroColuna2 = valorFiltro2;

                jsonValue.jsStoreQueryArtigoBeanRESTful.filtroColuna3NotNull = filtro3;
                jsonValue.jsStoreQueryArtigoBeanRESTful.valorfiltroColuna3 = valorFiltro3;
                ...
                $.ajax({
                    ...
                    success: function (data) {
                        ...
                        $.each(json_obj['jsStoreDadosArtigosBeanRESTful']['jsArtigosBeanRestFuls'], function (key, value) {
                            ...
                            var productInCart = getCookieCart();
                            var selectLote = null;
                            var selectRefImg =null;
                            var productLote =null ;
                            var loteInt = null;


                                if (productCart !== null ) { // product in cart
                                    if(productCart.CLoteInt === null){
                            ...
                            //função para mudar a cor de artigos com lotes no catalogo caso estejam dentro do carrinho
                            function changeFunc(i) {
                                      
                                var productInCart = getCookieCart();
                                i = i.split("%");

                                $.each(productInCart, function (k, u) {
                                var productLote = parseInt(u.CLoteInt);

                                if(productLote === null){
                                    console.log("Erro: Não tem Lotes");
                                }else{ 
                                    $.each(productInCart, function (x, y) {
                                        if(i[0] == y.CLoteInt){
                                            $('.product-item-' + value.CArtigo).css("background-color", "#ddd");
                                            return false;
                                        }else{
                                            $('.product-item-' + value.CArtigo).css("background-color", "#FFF");
                            ...
                            if(value.listaStkLotes != null){
                                selectLote = '<select id="lotesSel-' + value.CArtigo +'" class="form-control lote-combo">'
                                ...
                                $.each(value.listaStkLotes, function (key, valor) {
                                    ...
                                    loteInt = valor.CloteInt;
                                });
                                
                                $(document).on('change', '.lote-combo', function () {
                                     changeFunc($('#lotesSel-'+value.CArtigo+' option:selected').val());
                                });
                                selectLote +='</select>';
                            } 
                            ...
                            '<div class="thumbnail product-item-' + value.CArtigo+ (loteInt != null? loteInt : ' ') + + '" '+addedProductBackground+'>'+
                                html_imagem +
                                '<div class="caption">'+
        ...
        $(document).on('click', '.add-click', function () {
            ...
            $.each(catalogProducts, function (key, value) {
                ...
                if (productId == value.CArtigo) {
                    ...
                    //console.log(objProduct.CLoteInt, 11)
                    ...
                    console.log(objProduct.CLoteInt, 22)
            ...
            if (productQty > 0) {
                ...
                $.ajax({
                    ...
                    addProductToCart(objProduct, productId, objProduct.CLoteInt, objProduct.CLote);
        ...
        function searchCartItemById(codArtigo, cLote, cloteint ,refimagens) {
            
            var cookie_cart = getCookieCart(),
                productCartObject = null;

            //console.log(codArtigo, cLote, cloteint ,refimagens);

            if (cookie_cart.length > 0) {
                $.each(cookie_cart, function (key, value) {
                    if(refimagens == null || refimagens ==""){
                        if(codArtigo == value.codArtigo && (cloteint == null || cloteint =="")){
                            productCartObject = value;
                        } else if (codArtigo == value.codArtigo && cloteint == value.CLoteInt) {
                            productCartObject = value;
                        }
            ...
            //console.log(productCartObject, "productCartObject");
        ...
        */
        function addProductToCart(product, productId , CLoteInt,CLote){
            var cookie_cart = getCookieCart();

            if (searchCartItemById(productId, CLote, CLoteInt, product.refimagens) === null) {
        ...
        $(document).on('change', '.upVal', function (e) {
            ...
            if ($('#cart_page').hasClass('active')) {
                ...
                if ($.isNumeric(up_value)) {
                    ...
                    var productId = $(this).data('product-id'),
                        productQty = $(this).val(),
                        productCLote= $(this).data('product-lote'),
                        productCLoteInt= $(this).data('product-loteint'),
                        refimagens=$(this).data('product-refimg');
                        //console.log("CArtigo:",productId," Quantidade:",productQty, " CLote:",productCLote, " CLoteInt:",productCLoteInt, " refimagens:",refimagens);
                        if(productCLoteInt !== null){
                            objProduct = searchCartItemById(productId, productCLote, productCLoteInt, refimagens);
                        }else{
                            objProduct = searchCartItemById(productId);
                        }
        ...
        // nome do filtro tem de ser exatamente igual ao campo JPA senão entra na exceção e não vai trazer nenhum artigo
        let selectedtab,valorFiltro1,filtro1,valorFiltro2,filtro2,valorFiltro3,filtro3;

        $(document).on('click', '.nav-tabs a[href*="#cat"]', function () {

            var href = $(this).attr('href');
            var parts = href.split('#');
            var valorfiltro = parts[0].split('=')[1];
            ...
            if(valorfiltro==1){
                ...
                console.log("entrou 111")
                filtro1 = "<?php echo get_option('sss_filtro1');?>";
                valorFiltro1 = "<?php echo get_option('sss_valorfiltro1') ?>"
                filtro2 = "";
                valorFiltro2 = "";
                filtro3 = "";
                valorFiltro3 = "";
                ...
                filtro1 = "";
                valorFiltro1 = "";
                filtro2 = "<?php echo get_option('sss_filtro2'); ?>";
                valorFiltro2 = "<?php echo get_option('sss_valorfiltro2') ?>"
                filtro3 = "";
                valorFiltro3 = "";
                selectedtab = 'solidaria';
            }else if(valorfiltro==4){
                filtro1 = "";
                valorFiltro1 = "";
                filtro2 = "";
                valorFiltro2 = "";
                filtro3 = "<?php echo get_option('sss_filtro3'); ?>";
                valorFiltro3 = "<?php echo get_option('sss_valorfiltro3') ?>"
                selectedtab = 'solidaria';
            }
            console.log(valorfiltro)
            console.log("filtro 1: ",filtro1 ," valor filtro 1: ",valorFiltro1);
            console.log("filtro 2: ",filtro2 ," valor filtro 2: ",valorFiltro2);
            console.log("filtro 3: ",filtro3 ," valor filtro 3: ",valorFiltro3);
            tabs(valorfiltro);
        });

        function tabs(Solidaria){

            if(Solidaria == 3 || Solidaria == 4){
                ...
                ajax_call_to_table();
                check_btns_display('<?php echo get_option('sss_dd_select_name');?>', selectedtab);
...
//fim alteracoes
        ...
        function populateFrontendEncomendaArtigos () {
            ...
            if (checkoutItems.length > 0) {
                ...
                $.each(checkoutItems, function(key,value){
                    ...
                    var price_label = 'ct_price',background
                        price_desc = '';
                    ...
                    var valorr = value.qtd;
                    var valorrint= parseInt(valorr)
                    var background = ' ';
                   // console.log(checkoutItems)

                    $("#products_cart").append('' +
                        '<div class="item col-xs-12 col-sm-6 col-md-6 col-lg-4 row_number ' + display_class + '">'+
                            '<div id="product-carrinho-' + (value.CLoteInt != null? value.CLoteInt : value.codArtigo) +'" class="thumbnail">'+
                                ...
                                '<div class="caption">'+
                                    ...
                                    '<div class="row">'+
                                        '<div class="col-md-4 col-xs-12 col-sm-4">' + 
                                            '<input type="number" class="form-control math_cell cell_val_input upVal ct_qtd" data-product-id="' + value.codArtigo + '" data-product-lote="' + (value.CLote != null? value.CLote : '') + 
                                            '" data-product-loteint="' + (value.CLoteInt != null? value.CLoteInt : '') + '" value="' + valorrint + '" placeholder="'+ value.qtd_caixa +'" min="'+ value.qtd_caixa +'" maxlength="11"' + 
                                            'data-product-refimg="' + (value.refimagens != null ? value.refimagens : '') +'" ' + 'step='+value.qtd_caixa + '>' + 
                                        '</div>'+
                                        '<div class="col-md-8 col-xs-12 col-sm-8">' +
                                            (value.CLote == null? value.refimagens == null? '' : '<span class="ct_refimagem">' + value.refimagens + '</span>': '<span class="ct_lote">' + value.CLote + '</span>')+
                                        '</div>'+
                                    '</div>'+
                                    '<div id="quantidade-'+(value.CLoteInt == null ? '-'+ value.codArtigo : '-' + value.CLoteInt)+'" class="row" style="padding-top: 10px; padding-left: 15px;>' +
                                            '<div id="qtdisponivel">'+ '‍' +'</div>' + //dentro do espaço tem um caracter invisivel para os artigos no carrinho não ficarem deformados
                                    '</div>' +
                                    '<br/><div class="row">'+
        ...
        var aux = cartProducts.length;
        var aux2 = 0;
        validaStocks = false; //!"#
        if(validaStocks == true){
        $.each(cartProducts, function (key, value) { 
        
        if(value.CLoteInt !== null){
        console.log("entrou com lotes", value.CLoteInt);
        service_path="/seam/resource/rest/artigosStockService/getLoteIntArtigo/";

        $.ajax({
                type: "GET",
                url: req_url + service_path + req_conn + ':' + req_username + ':' + req_password + '/' + sigaStoreUsername + '/' + btoa(sigaStorePassword) + '/' + value.CLoteInt + '/' + value.codArtigo + '/' + sigaStockArm,
                dataType: 'json',
                headers: {
                    "Authorization": "Basic " + btoa(sigaStoreUsername + ":" + sigaStorePassword)
                },
                ...
                //console.log("entrou no serviço: ",data.lotesBeanRESTful.checkStatusRestFul);
                let hasSigaStockArm = false;
                let k = 0;
                var lista = null;
                if(data.lotesBeanRESTful.checkStatusRestFul.toLowerCase().includes("ok")){
                    $.each(data['lotesBeanRESTful']['listaStkLotes'], function (key, bean) {
                        console.log(bean);
                        if(!(bean.CArtigo == "")){
                            if(bean.CArtigo != null){
                                if(bean.qtdDispLote > 0){
                                    aux2++;
                                    hasSigaStockArm = true;
                                    if(value.qtd > bean.qtdDispLote){
                                        vaiProcessar++;
                                        ArtigoSemStock(bean,bean.qtdDispLote,vaiProcessar);
                                    }else if (aux2 == aux && vaiProcessar == 0) {
                                        encomenda();
                                    }
                                }
                                if(hasSigaStockArm == false){
                                    vaiProcessar++;
                                    ArtigoSemStock(bean,0,vaiProcessar);
                                }    
                            }else{
                                console.log("Código do Artigo Null");
                            }
                        }else{
                            //Objeto em Branco
                        }
                    ...
                    vaiProcessar++;
                    ArtigoSemStock(value,0,vaiProcessar);
                }
                    setTimeout(function () {
                        stopLoader();
                    }, timeout_delay)
           ...
           //sem lotes
        console.log("entrou sem lotes");
        service_path="/seam/resource/rest/artigosStockService/obterArtigoInJson/";
        $.ajax({
                type: "GET",
                url: req_url + service_path + req_conn + ':' + req_username + ':' + req_password + '/' + sigaStoreUsername + '/' + btoa(sigaStorePassword) + '/' + value.codArtigo,
                dataType: 'json',
                headers: {
                    "Authorization": "Basic " + btoa(sigaStoreUsername + ":" + sigaStorePassword)
                },
                ...
                success: function (data) {
                    ...
                    let k = 0;
                    var lista = null;
                    //console.log(data.jsRetornoArtigoBeanRestFul.checkStatusRestful);
                    if(data.jsRetornoArtigoBeanRestFul.checkStatusRestful.toLowerCase().includes("ok")){
                        $.each(data['jsRetornoArtigoBeanRestFul'], function (key, bean) {
                            if(bean.qtdDisponivel > 0){
                                lista = bean.listaQtdArm;
                                //console.log(bean);
                                if(lista !== null){
                                    for(k = 0;k < lista.length; k++){
                                        if(lista[k].CArmazem == sigaStockArm){
                                            aux2++;
                                            hasSigaStockArm = true;
                                            if(value.qtd > lista[k].quantidade){
                                                vaiProcessar++;
                                                ArtigoSemStock(bean,lista[k].quantidade,vaiProcessar);
                                            }else if (aux2 == aux && vaiProcessar == 0) {
                                                encomenda();
                                            }
                                        }
            ...
            //console.log("entrou enc");
            encomenda();
        }

        ////// função auxiliar para stocks
        function ArtigoSemStock(bean,qtddisponivel,vaiProcessar) { 
            //console.log(bean);
            if(vaiProcessar===1){
                toastr.warning("Um ou mais Produtos não tem Stock suficiente Disponível");
            }
            
            if(bean.CLoteInt != null){
                var productItem = document.getElementById("product-carrinho-"+(bean.CLoteInt != null? bean.CLoteInt : bean.CArtigo));
                var divElement = document.getElementById("quantidade-"+(bean.CLoteInt == null ? '-'+ bean.CArtigo : '-' + bean.CLoteInt));
            }else{
                var productItem = document.getElementById("product-carrinho-"+(bean.CLoteInt != null? bean.CLoteInt : bean.CArtigo));
                var divElement = document.getElementById("quantidade-"+(bean.CLoteInt == null ? '-'+ bean.CArtigo : '-' + bean.CLoteInt));
            }
            ...
            if(qtddisponivel !=null && qtddisponivel > 0){
                divElement.textContent = "Quantidade Disponivel: " + qtddisponivel;
            }else{
                divElement.textContent = "Artigo sem Stock";
            }
            productItem.setAttribute("style", "background-color: rgba(188, 143, 143, 0.5);");
        }


        // função da realização da encomenda
        function encomenda(){//!"#
            //console.log("aux: ",aux," aux2: ",aux2, " encomenda feita");
            //console.log("realizou a encomenda");
            //vai processar a encomenda
            var objPost = [];
            ...
            $.ajax({
                ...
                success: function(response) {
                    ...
                    if (response.encomendasClienteBeanRESTful.checkStatusRESTful.includes('9-0') === true) {
                        ...
                        console.log(response.encomendasClienteBeanRESTful.nrEncomenda);
...
<ul class="nav nav-tabs">
    ...
    <li id="catalog_page"><a data-toggle="tab" href="?valor=1#cat">
    ...
    <!-- Inicio | Novos Tabs 23/02/2023--> 
    <!-- Novo Codigo href="#artg" -->
    
    <!-- tab 1 -->
    <?php 
        $filtro1 = get_option('sss_filtro1');#tab_1 MARCAS.CODIGO
        if($filtro1 !== ''){
    ?>
    <style>
        #tab_1 a {
            background-color:<?php echo get_option('sss_background1'); ?>;
            opacity:<?php echo get_option('sss_opacity1'); ?>;
            color:<?php echo get_option('sss_color1'); ?>;
            border: 1.8px solid grey;
        }
    </style>
    <li id="tab_1">
        <a data-toggle="tab" href="?valor=2#cat">
            <img class="custom-icon-campanha" src="<?php echo get_option('sss_pathtab1'); ?>">
            <span class="text_tab"><?php echo get_option('sss_nometab1'); ?></span>
        </a>
    </li>
    <?php } ?>
    <!-- tab 2 -->
    <?php 
        $filtro2 = get_option('sss_filtro2');#tab_2 GESTOR
        if($filtro2 !== ''){
    ?>
    <style>
        #tab_2 a {
            background-color:<?php echo get_option('sss_background2'); ?>;
            opacity:<?php echo get_option('sss_opacity2'); ?>;
            color:<?php echo get_option('sss_color2'); ?>;
            border: 1.8px solid grey;
        }
    </style>
    <li id="tab_2">
        <a data-toggle="tab" href="?valor=3#cat">
            <img class="custom-icon-solidaria" src="<?php echo get_option('sss_pathtab2'); ?>">
            <span class="text_tab"><?php echo get_option('sss_nometab2'); ?></span>
        </a>
    </li>
    <?php } ?>
    <!-- tab 3 -->
    <?php 
        $filtro3 = get_option('sss_filtro3');# MARCAS.CODIGO
        if($filtro3 !== ''){
    ?>
    <style>
        #tab_3 a {
            background-color:<?php echo get_option('sss_background3'); ?>;
            opacity:<?php echo get_option('sss_opacity3'); ?>;
            color:<?php echo get_option('sss_color3'); ?>;
            padding: 5.6px 20px 5.6px 15px;
            border: 1.8px solid grey;
        }
    </style>
    <li id="tab_3">
        <a data-toggle="tab" href="?valor=4#cat">
            <img class="custom-icon-fedor" src="<?php echo get_option('sss_pathtab3'); ?>">
            <span class="text_tab"><?php echo get_option('sss_nometab3'); ?></span>
        </a>
    </li>
    <?php } ?>

     <!-- Fim | Novos Tabs 23/02/2023 -->
...
<div class="tab-content">
    ...
    <div id="cat" class="tab-pane fade">
        ...
        <div id="pesqui" class="search-panel mt10">
            <div class="row">
                <div class="col-md-3">
